...
const EditOffer = ({ createNotification }) => {
  ...
  const handleEditOffer = async () => {
    ...
    try {
      await axios.put(
        '/api/company/edit-offer/' + offerId,
        offerData,
        { headers: { 'X-CSRF-TOKEN': csrfAccessToken, } },
      );
      createNotification(successEditOfferNotification);
      history.push('/offers');
    } catch (error) {
  ...
  const handleCancelEditOffer = () => history.push('/offers');
  ...
  const getOfferData = async () => {
    try {
      const response = await axios.get('/api/company/get-offer-data/' + offerId);
      dispatchOfferData(response.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    if (offerData.offerRate !== "" && Object.keys(offersTypes).length !== 0)
      setOfferRateName(offersTypes[offerData.offerRate - 1].name);
  }, [
    offerData,
    offersTypes
  ])
  ...
  return (
    <Container maxWidth="md">
      <Box my={4}>
        <Typography variant="h4" align="center">Editar oferta</Typography>
      </Box>
      {loading
        ?
